...
<body>
    ...
    echo '<h2>' . __FILE__ . '</h2>';
    echo '<h2>' . DEFAULT_INCLUDE_PATH . '</h2>';

    print_r($_GET);
// Norimas rezultatas Sveiki Kazimiras. Kaip laikosi Kaunas?
    ...
    print_r($_GET);
    print_r(isset($_GET['first_name']));
    print_r(isset($_GET['city']));
    if ($_GET['first_name'] && $_GET['city']) {
        echo '<h2>Sveiki ' . $_GET['first_name'] . '. Kaip laikosi ' . $_GET['city'] . '?</h2>';
        // Sveiki Kazimiras. Kaip laikosi Kaunas?
    }
